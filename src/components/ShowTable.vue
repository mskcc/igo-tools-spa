    ...
    <md-table-row v-for="tool in tools" :key="tool._id.$oid" :tool="tool">
      ...
      <md-table-cell class="doc-col">
        ...
        <div v-if="tool.documentation.includes(',')" class="tool-documentation">
          <span
            v-for="(item, index) in tool.documentation.split(',')"
            :key="index"
          >
            <a :href="item">
              <i
                v-if="item.includes('github')"
                class="fa-icon fab fa-github fa-3x"
              ></i>
              <i
                v-if="item.includes('plvpipetrack1')"
                class="fa-icon fas fa-book fa-3x"
              ></i>
        ...
        <div v-else>
          <a :href="item"
            ><i class="fa-icon fas fa-link fa-2x"></i>
            <md-tooltip md-delay="300">{{ item }}</md-tooltip>
          </a>
        </div>
      </md-table-cell>
      ...
      <md-table-cell class="edit-col">
        <md-button
          ...
          v-on:click="handleEdit(tool)"
        >
          <md-icon>edit</md-icon>
        </md-button>
        <md-button
          class="md-icon-button md-raised"
          v-on:click="handleDelete(tool._id.$oid)"
        >
